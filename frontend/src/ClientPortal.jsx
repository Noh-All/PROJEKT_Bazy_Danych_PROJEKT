    ...
    setTimeout(async () => {
      ...
      try {
        ...
        if (odpowiedz.ok) {
          ...
          setMojeBilety([...mojeBilety, biletGotowy]);
                  ...
                  <div key={idx} className="border border-gray-300 p-6 rounded-2xl flex flex-col md:flex-row gap-6 items-center bg-gray-50 shadow-sm relative">
                    <div className="flex-1 w-full space-y-4 border-r border-dashed border-gray-300 pr-6">
                      ...
                      {bilet.kursy.map((kurs, index) => (
                        <div key={kurs.id_kursu} className="bg-white p-3 rounded border text-sm">
                          <div className="font-bold">{kurs.skad} ➔ {kurs.dokad}</div>
                          <div className="text-gray-500">KOD BILETU DO ZESKANOWANIA:</div>
                          <div className="bg-gray-100 text-xs font-mono p-1 mt-1 break-all select-all">{bilet.kody_qr && bilet.kody_qr[index]}</div>
                        </div>
                      ))}
                    </div>
